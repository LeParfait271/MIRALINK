...
- Version initiale : `0.1.0`.
- Chaque commit qui modifie le site ou l'application augmente la version publique de `0.01` : `0.10` → `0.11` → `0.12` → … → `0.57` → `0.58` → `0.59`. La version technique du paquet npm utilise `0.59.0` pour respecter le format semver, tandis que la version affichée et livrée du site reste `0.59`.
- La version affichée du firmware est toujours exactement celle du site. Une nouvelle version implique un build firmware documenté et un UF2 dont la métadonnée embarquée est identique à la version publique. CMake peut utiliser la forme technique `0.59.0`, mais le firmware affiché et livré reste `0.59`.
...
- `2026-08-16` — Le correctif firmware `0.56` reprend le séquencement DS5Dongle de reconnexion passive : requête ACL Classic de classe gamepad acceptée explicitement, inquiry arrêté avant authentification/chiffrement, puis HID ouvert par la manette. La source et le firmware DS5Dongle ont été utilisés comme référence comportementale ; aucun code ou binaire n'a été copié. La reconnexion 0.56 reste non validée sur matériel et le score reste `54,4 %`.
- `2026-08-16` — Le candidat `0.57` retire la condition d'adresse mémorisée de la requête ACL gamepad, comme DS5Dongle : l'ACL est acceptée, puis la clé/SSP et le rapport HID strict décident de la confiance. La reconnexion reste non validée sur matériel et le score reste `54,4 %`.
- `2026-08-16` — Le candidat `0.58` retire aussi le refus HID prématuré fondé sur la fenêtre d'appairage ou le cache RAM. Après un ACL gamepad accepté, le descripteur et le rapport enrichi 0x31 avec CRC restent les barrières fonctionnelles. Aucun test matériel supplémentaire n'est demandé dans ce lot.
- `2026-08-16` — Le candidat `0.59` aligne SSP/PIN, suppression de clé après échec d'authentification et timeout ACL sans CID HID sur le cycle DS5Dongle. Le comportement matériel reste à valider une seule fois après ce gel.
