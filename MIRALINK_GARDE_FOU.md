...
- Version publique actuelle du site : `0.25`
...
- Version initiale : `0.1.0`.
- Chaque commit qui modifie le site ou l'application augmente la version publique de `0.01` : `0.10` → `0.11` → `0.12` → … → `0.24` → `0.25`. La version technique du paquet npm utilise `0.25.0` pour respecter le format semver, tandis que la version affichée et livrée du site reste `0.25`.
