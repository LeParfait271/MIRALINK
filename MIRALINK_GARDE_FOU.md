...
- Version initiale : `0.1.0`.
- Chaque commit qui modifie le site ou l'application augmente la version publique de `0.01` : `0.10` → `0.11` → `0.12` → … → `0.54` → `0.55` → `0.56`. La version technique du paquet npm utilise `0.56.0` pour respecter le format semver, tandis que la version affichée et livrée du site reste `0.56`.
- La version affichée du firmware est toujours exactement celle du site. Une nouvelle version implique un build firmware documenté et un UF2 dont la métadonnée embarquée est identique à la version publique. CMake peut utiliser la forme technique `0.56.0`, mais le firmware affiché et livré reste `0.56`.
