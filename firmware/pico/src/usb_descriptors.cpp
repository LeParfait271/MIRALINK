...
constexpr std::uint8_t kBridgeReportDescriptor[] = {
    ...
    // MiraLink maintenance traffic remains a distinct top-level vendor
    // collection, but shares the controller interface and endpoints. Report
    // IDs keep it unambiguous without creating a second Sony-matched HID
    // interface on Linux.
    HID_USAGE_PAGE_N(HID_USAGE_PAGE_VENDOR, 2),
    HID_USAGE(0x01),
    HID_COLLECTION(HID_COLLECTION_APPLICATION),
