...
import { applyTranslations, translate } from './i18n.js?ui=57-acl-admission';
...
import { scrollToRouteTarget } from './site-effects.js?ui=57-acl-admission';
...
const state = {
  ...
  savedConfig: null,
  logs: logStore.get(),
  version: { version: '0.57', developer: 'MaruChiwa', lastUpdated: '2026-08-16' }
};
...
async function loadMetadata() {
  try {
    const response = await fetch('./build-info.json?ui=57-acl-admission', { cache: 'no-store' });
...
function init() {
  ...
  if ('serviceWorker' in navigator) navigator.serviceWorker.register('./sw.js?ui=57-acl-admission').catch((error) => addLog('info', `Offline shell unavailable: ${error.message}`));
  loadMetadata();
