...
import { applyTranslations, translate } from './i18n.js?ui=61-incoming-report-handshake';
...
import { scrollToRouteTarget } from './site-effects.js?ui=61-incoming-report-handshake';
...
const state = {
  ...
  savedConfig: null,
  logs: logStore.get(),
  version: { version: '0.61', developer: 'MaruChiwa', lastUpdated: '2026-08-16' }
};
...
async function loadMetadata() {
  try {
    const response = await fetch('./build-info.json?ui=61-incoming-report-handshake', { cache: 'no-store' });
...
function init() {
  ...
  if ('serviceWorker' in navigator) navigator.serviceWorker.register('./sw.js?ui=61-incoming-report-handshake').catch((error) => addLog('info', `Offline shell unavailable: ${error.message}`));
  loadMetadata();
