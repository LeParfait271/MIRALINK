...
import { applyTranslations, translate } from './i18n.js?ui=48-discoverable-reconnect';
...
import { scrollToRouteTarget } from './site-effects.js?ui=48-discoverable-reconnect';
...
const state = {
  ...
  savedConfig: null,
  logs: logStore.get(),
  version: { version: '0.48', developer: 'MaruChiwa', lastUpdated: '2026-08-15' }
};
...
async function loadMetadata() {
  try {
    const response = await fetch('./build-info.json?ui=48-discoverable-reconnect', { cache: 'no-store' });
...
function init() {
  ...
  if ('serviceWorker' in navigator) navigator.serviceWorker.register('./sw.js?ui=48-discoverable-reconnect').catch((error) => addLog('info', `Offline shell unavailable: ${error.message}`));
  loadMetadata();
