...
import { applyTranslations, translate } from './i18n.js?ui=58-hid-admission';
...
import { scrollToRouteTarget } from './site-effects.js?ui=58-hid-admission';
...
const state = {
  ...
  savedConfig: null,
  logs: logStore.get(),
  version: { version: '0.58', developer: 'MaruChiwa', lastUpdated: '2026-08-16' }
};
...
async function loadMetadata() {
  try {
    const response = await fetch('./build-info.json?ui=58-hid-admission', { cache: 'no-store' });
...
function init() {
  ...
  if ('serviceWorker' in navigator) navigator.serviceWorker.register('./sw.js?ui=58-hid-admission').catch((error) => addLog('info', `Offline shell unavailable: ${error.message}`));
  loadMetadata();
