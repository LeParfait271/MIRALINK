...
import { applyTranslations, translate } from './i18n.js?ui=54-tombstone-rearm';
...
import { scrollToRouteTarget } from './site-effects.js?ui=54-tombstone-rearm';
...
const state = {
  ...
  savedConfig: null,
  logs: logStore.get(),
  version: { version: '0.54', developer: 'MaruChiwa', lastUpdated: '2026-08-15' }
};
...
async function loadMetadata() {
  try {
    const response = await fetch('./build-info.json?ui=54-tombstone-rearm', { cache: 'no-store' });
...
function init() {
  ...
  if ('serviceWorker' in navigator) navigator.serviceWorker.register('./sw.js?ui=54-tombstone-rearm').catch((error) => addLog('info', `Offline shell unavailable: ${error.message}`));
  loadMetadata();
