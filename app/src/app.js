...
import { applyTranslations, translate } from './i18n.js?ui=56-acl-reconnect';
...
import { scrollToRouteTarget } from './site-effects.js?ui=56-acl-reconnect';
...
const state = {
  ...
  savedConfig: null,
  logs: logStore.get(),
  version: { version: '0.56', developer: 'MaruChiwa', lastUpdated: '2026-08-16' }
};
...
async function loadMetadata() {
  try {
    const response = await fetch('./build-info.json?ui=56-acl-reconnect', { cache: 'no-store' });
...
function init() {
  ...
  if ('serviceWorker' in navigator) navigator.serviceWorker.register('./sw.js?ui=56-acl-reconnect').catch((error) => addLog('info', `Offline shell unavailable: ${error.message}`));
  loadMetadata();
