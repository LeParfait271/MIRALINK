...
import { applyTranslations, translate } from './i18n.js?ui=49-inactivity-activity';
...
import { scrollToRouteTarget } from './site-effects.js?ui=49-inactivity-activity';
...
const state = {
  ...
  savedConfig: null,
  logs: logStore.get(),
  version: { version: '0.49', developer: 'MaruChiwa', lastUpdated: '2026-08-15' }
};
...
async function loadMetadata() {
  try {
    const response = await fetch('./build-info.json?ui=49-inactivity-activity', { cache: 'no-store' });
...
function init() {
  ...
  if ('serviceWorker' in navigator) navigator.serviceWorker.register('./sw.js?ui=49-inactivity-activity').catch((error) => addLog('info', `Offline shell unavailable: ${error.message}`));
  loadMetadata();
