...
import { applyTranslations, translate } from './i18n.js?ui=55-encryption-gate';
...
import { scrollToRouteTarget } from './site-effects.js?ui=55-encryption-gate';
...
const state = {
  ...
  savedConfig: null,
  logs: logStore.get(),
  version: { version: '0.55', developer: 'MaruChiwa', lastUpdated: '2026-08-15' }
};
...
async function loadMetadata() {
  try {
    const response = await fetch('./build-info.json?ui=55-encryption-gate', { cache: 'no-store' });
...
function init() {
  ...
  if ('serviceWorker' in navigator) navigator.serviceWorker.register('./sw.js?ui=55-encryption-gate').catch((error) => addLog('info', `Offline shell unavailable: ${error.message}`));
  loadMetadata();
