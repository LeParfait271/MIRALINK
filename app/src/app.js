...
import { applyTranslations, translate } from './i18n.js?ui=41-hid-action-state';
...
import { scrollToRouteTarget } from './site-effects.js?ui=41-hid-action-state';
...
const state = {
  ...
  savedConfig: null,
  logs: logStore.get(),
  version: { version: '0.43', developer: 'MaruChiwa', lastUpdated: '2026-08-15' }
};
...
async function loadMetadata() {
  try {
    const response = await fetch('./build-info.json?ui=41-hid-action-state', { cache: 'no-store' });
...
function init() {
  ...
  if ('serviceWorker' in navigator) navigator.serviceWorker.register('./sw.js?ui=41-hid-action-state').catch((error) => addLog('info', `Offline shell unavailable: ${error.message}`));
  loadMetadata();
