...
import { applyTranslations, translate } from './i18n.js?ui=47-auth-recovery';
...
import { scrollToRouteTarget } from './site-effects.js?ui=47-auth-recovery';
...
const state = {
  ...
  savedConfig: null,
  logs: logStore.get(),
  version: { version: '0.47', developer: 'MaruChiwa', lastUpdated: '2026-08-15' }
};
...
async function loadMetadata() {
  try {
    const response = await fetch('./build-info.json?ui=47-auth-recovery', { cache: 'no-store' });
...
function init() {
  ...
  if ('serviceWorker' in navigator) navigator.serviceWorker.register('./sw.js?ui=47-auth-recovery').catch((error) => addLog('info', `Offline shell unavailable: ${error.message}`));
  loadMetadata();
