...
import { applyTranslations, translate } from './i18n.js?ui=42-hero-layout';
...
import { scrollToRouteTarget } from './site-effects.js?ui=42-hero-layout';
...
const state = {
  ...
  savedConfig: null,
  logs: logStore.get(),
  version: { version: '0.44', developer: 'MaruChiwa', lastUpdated: '2026-08-15' }
};
...
async function loadMetadata() {
  try {
    const response = await fetch('./build-info.json?ui=42-hero-layout', { cache: 'no-store' });
...
function init() {
  ...
  if ('serviceWorker' in navigator) navigator.serviceWorker.register('./sw.js?ui=42-hero-layout').catch((error) => addLog('info', `Offline shell unavailable: ${error.message}`));
  loadMetadata();
