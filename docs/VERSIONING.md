...
```text
0.10 → 0.11 → 0.12 → … → 0.39 → 0.40 → 0.41 → 0.42 → 0.43 → 0.44 → 0.46 → 0.47 → 0.48 → 0.49 → 0.50 → 0.51
```

The current sequence continues through public version `0.51`.
...
`app/package.json` uses the valid npm semver representation `0.51.0`; this is
packaging metadata for the public site version `0.51`, not a separate product
release.
...
UF2 metadata, release folder and manifest use `0.51`. CMake uses the technical
form `0.51.0` only because it requires three numeric segments; the firmware
reported by the device remains exactly `0.51`.

The current local delivery manifest is `docs/DELIVERY_MANIFEST.json`.

The current site and firmware version is `0.51`. It contains the clean-room,
