...
## Current comparison — MiraLink 0.42

| Capability | Weight | DS5Dongle | MiraLink 0.41 candidate | MiraLink 0.42 candidate | New evidence |
| --- | ---: | ---: | ---: | ---: | --- |
| USB persona / host compatibility | 20% | 100% | 72% | 72% | No descriptor change or new host proof |
| Bluetooth pairing / reconnect | 20% | 100% | 55% | 55% | 0.40 hardware showed radio `PASS` but the known DualSense stayed offline; 0.41 close-rearm and 0.42 idle-resume rearm are software-only |
...
| Configuration / diagnostics | 7.5% | 100% | 82% | 82% | No new hardware capability proof; 0.42 only queues Bluetooth recovery after a configuration-driven idle resume |
| **Weighted proven score** | **100%** | **100%** | **54.4%** | **54.4%** | **No score increase without a new hardware retest** |

MiraLink 0.42 keeps `76%` raw source coverage. The weighted total is computed
...
page-scan corrections remain discounted until the next manual flash/test.
...
MiraLink 0.42 size is recorded from the frozen release build. Size alone says
