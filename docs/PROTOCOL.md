...
experimental DualSense-family interface. Firmware 0.49 uses
...
The binary frame protocol remains version `1` in firmware `0.49`; no report ID,
...
Firmware `0.49` returns exactly two payload bytes after a successful
`COMMIT_CONFIG`:
...
claim that a controller is connected. Firmware 0.49 exposes one HID-only USB
...
`GET_CONTROLLER_STATE` returns a 48-byte payload for schema `2`. Firmware 0.49
...
After accepting a valid Bluetooth HID descriptor, firmware `0.49` advances a
